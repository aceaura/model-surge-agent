...
上游发来的每一帧都要先进聚合器：非流式靠它把事件收拢成一次性响应，流式也要靠它算 usage 与判残缺（`pipeline/bridge.go` 的 `agg.Add(ev)` 对每个事件无条件执行）。于是「上游发多少」直接决定了本服务在这条路径上花多少 CPU 与内存，而此前这里既没有摊还的拼接形态，也没有任何整条流的上限。

#### 6.32.1 二次复杂度不是理论问题

聚合器此前往 `Block` 的字符串字段上 `+=`：`b.Text += ev.Text`。每帧一次全量重分配，n 帧拼出 L 字节要付 O(L·n)。本机探针实测：**160000 帧（10 MiB 文本）耗时 3 分 03 秒、累计分配 763 GiB**。这不是尾部延迟变差，是一次正常的长响应把一个 CPU 核占满三分钟——而流式客户端此时已经拿到了全部内容，多花的三分钟纯粹是聚合器在后面追。

改成每块四个 `strings.Builder`（text / thinking / signature / tool input）后同一条探针是 **14.9 毫秒、49 MiB**，约 12200 倍。

Builder 不能挂在 `Block` 上：`Block` 要原样序列化进下一轮请求，不该携带解码过程的状态（与既有的 `sawInput` 同一条理由）。因此另立 `acc` 结构、与 `blocks` 同键存在 `accs map[int]*acc` 里。

**物化用赋值而非追加**，于是天然幂等。这一点是必需的而不是顺手：`Response()` 在一次请求里会被调用多次（`bridge` 判完 `StopReason` 之后 `writeSuccess` 里还会再调一次），而三个出口 `Response` / `IncompleteTools` / `UnsafeToClose` 各自都要先物化——后两者读的是 `ToolUse.Input` 与 `Thinking.Signature`，不物化就看到空串，于是**每个带入参的工具调用都会被判成截断**，整条响应被当成残缺。代价是 Builder 必须始终持有全量、中途不能交出所有权；`String()` 不复制底层字节，重复调用的开销可忽略。

`EvBlockStart` 那一路有两处易错。一是上游复用同一索引开新块时累积器必须 `delete`，否则新块继承上一个块的内容（`blocks` 那一行是整体替换，累积器不跟着换就对不上）。二是块开始事件可能自带内容，它要先写进 Builder——物化是赋值式的，Builder 里不是全量就会把这部分抹掉。

**同类形态还有三处**，一并改：`codec/chatcompletions/decode_stream.go` 的 `toolSlot.pending`（分片数同样由上游决定）、`encode_request.go` 的 `thinking` 与文本拼接两处。前者是子代理那轮漏报的第四处，靠 `grep -rn '+='` 全量复核才找出来。

#### 6.32.2 三道闸门

摊还之后单位成本降下来了，总量仍然无界：块索引是上游给的、本服务不校验它，一个索引跳号的 bug 会变成本服务的 map 无界增长；累计字节同理——单帧上限（`codec/sse.go` 的 `maxFrameBytes`）只管一帧，乘以无限帧数等于没有上限。实测修复前 64 × 1 MiB = 64 MiB 的累积不触发任何检查。

三道闸门，阈值各有出处：

| 闸门 | 常量 | 值 | 出处 |
|---|---|---|---|
| 累计内容字节 | `ir.maxContentBytes` | 32 MiB | 与非 SSE 整份响应上限（`pipeline/upstream.go` 的 `maxWholeResponseBytes`）**同值**：同一份内容换个传输形态不该拿到更宽松的预算 |
| 内容块数 | `ir.maxBlocks` | 4096 | 四协议的真实块数是「个」到「十几个」的量级，三个数量级的余量 |
| 工具槽位数 | `chatcompletions.maxToolSlots` | 4096 | 同上；真实并行调用数是「个」到「几十个」 |

后两者同值但**各包一份常量**：让 codec 去 import `ir` 的未导出常量做不到，而导出它会把一个内部判据变成跨包契约。

字节记账用独立字段 `contentBytes` 累加，不遍历所有 Builder 求和——后者是每帧一次 O(块数) 的遍历，把刚消掉的二次复杂度又装回来。块开始事件自带的内容也要记账，否则一个「每个块都自带 1 MiB 正文」的上游就绕过了字节闸门。

**撞线之后立即停止累积**，而不是「收完再判」：继续累下去等于上限没有意义。这一条在变异验证里暴露了一个测试缺口——把 `admit` 开头的早返回删掉时测试全绿，因为既有断言撞的都是字节闸门，那时累计值本身已越线，看不出「置位即停」被删。补一格「撞块数闸门时累计字节还很小」才守住。

闸门做成查询（`Overflow() string`）而不是让 `Add` 返回错误：`bridge` 在补「按 `max_tokens` 截断」的终止事件时也会调 `Add`，那是本服务自造的事件、不该被闸门挡住，而给 `Add` 加返回值会让那处也必须处理一个永不发生的错误。返回值非空时是一句可直接进错误消息的说明，**含闸门名与阈值、不含任何内容片段**——超限响应的内容进日志正是这道闸门要防的事。

#### 6.32.3 数据面的处置

`ErrUpstream` + `response_too_large`，**显式不可重试**：换个目标重新生成一遍还是同样的规模，而重试会把这笔代价再付一次。

判定排在提交处理**之后**。首帧就撞线时（单帧上限恰好顶到累计上限）仍要走完提交流程，否则流式客户端拿到的是一个连响应头都没有的断连。`finish` 已按 `committed` 分流（未提交→HTTP 错误信封，已提交→流内错误帧），这里不必再判一遍。

工具槽位那道闸门在 codec 里，因此 `decodeToolCalls` 的签名从 `[]ir.Event` 改成 `([]ir.Event, error)`。撞线时**报错而不是静默跳过**：跳过会让客户端收到一份少了若干工具调用的 HTTP 200，而那些调用上游确实决定要发。

#### 6.32.4 明确不做（已核实，勿再提）

- **不改单帧上限**：`maxFrameBytes` 管的是一帧，与累计闸门是两个维度，同时存在才完整。
- **三道阈值不做成可配**：能配就能配坏，而这三个值对真实响应都有三个数量级的余量。
- **不按块分别限字节**：上游的块划分是它的自由，限总量才是本服务真正关心的资源。
- **不做 usage 驱动的上限**：`usage` 是上游自报的，用它当闸门等于让被限的一方自己决定限不限。
- **不照搬参考仓库**：三仓正文都用 Builder/`push_str`，但两家 Go 仓的**工具入参仍是 `+=`**；整条流的累计字节/帧数/块数闸门**三仓全无**。这一轮的阈值与措辞都是自定的。
...
| 2.32 | 2026-09-20 | 响应侧累积的资源界限首次成文（[6.32](#632-响应侧累积的资源界限)）：一处复杂度修复加三道闸门，全在中立表示、转换层与数据面内，无新增端点、无新增列、无库变更。**行为变更一**：聚合器此前往 `Block` 的字符串字段上 `+=`，n 帧拼 L 字节付 O(L·n)；而这条路径**每个请求都走**——非流式靠它收拢响应，流式也要靠它算 usage 与判残缺（`bridge` 的 `agg.Add(ev)` 无条件执行）。本机探针实测 160000 帧（10 MiB 文本）耗时 **3 分 03 秒、累计分配 763 GiB**：一次正常的长响应把一个核占满三分钟，而流式客户端早已收完内容。改成每块四个 `strings.Builder`（text/thinking/signature/tool input）后同一探针 **14.9 毫秒、49 MiB**，约 12200 倍。Builder 不挂在 `Block` 上（它要原样序列化进下一轮请求，不该带解码状态，与既有 `sawInput` 同理），另立 `acc` 与 `blocks` 同键。**物化用赋值而非追加**因而天然幂等，这是必需的：`Response()` 一次请求里会被调多次（`writeSuccess` 里还有一次），而 `Response`/`IncompleteTools`/`UnsafeToClose` 三个出口各自都要先物化——后两者读 `ToolUse.Input` 与 `Thinking.Signature`，不物化就看到空串，**每个带入参的工具调用都会被判成截断**。代价是 Builder 必须始终持有全量、中途不交出所有权。两处易错一并处置：上游复用同一索引开新块时累积器必须 `delete`（否则新块继承上一块内容），块开始事件自带的内容要先写进 Builder（赋值式物化会抹掉不在 Builder 里的部分）。**同类形态另有三处**改掉：`chatcompletions` 的 `toolSlot.pending`（子代理那轮漏报的第四处，靠 `grep -rn '+='` 全量复核找出）、`encode_request.go` 的 `thinking` 与文本拼接。**行为变更二**：摊还之后总量仍无界——块索引是上游给的且本服务不校验它，单帧上限只管一帧、乘无限帧数等于没有上限（实测 64 × 1 MiB 的累积不触发任何检查）。新增三道闸门：`ir.maxContentBytes` = 32 MiB（与非 SSE 整份响应上限 `maxWholeResponseBytes` **同值**：同一份内容换个传输形态不该拿到更宽松的预算）、`ir.maxBlocks` = 4096、`chatcompletions.maxToolSlots` = 4096（后两者同值但**各包一份常量**：codec 无法 import `ir` 的未导出常量，而导出它会把内部判据变成跨包契约）。字节记账用独立字段累加而非遍历所有 Builder 求和（后者是每帧一次 O(块数) 遍历，把刚消掉的二次复杂度装回来）；块开始事件自带的内容也记账（否则「每块自带 1 MiB 正文」的上游绕过闸门）；**撞线后立即停止累积**而非收完再判。闸门做成查询 `Overflow() string` 而不是让 `Add` 返回错误：`bridge` 补「按 `max_tokens` 截断」的终止事件时也调 `Add`，那是本服务自造的事件、不该被挡，而加返回值会让那处必须处理一个永不发生的错误。说明**含闸门名与阈值、不含任何内容片段**——超限内容进日志正是这道闸门要防的事。**行为变更三**：数据面处置为 `ErrUpstream` + `response_too_large` 且**显式不可重试**（换目标重新生成还是同样规模，重试把代价再付一次）；判定排在提交处理**之后**（首帧就撞线时仍要走完提交流程，否则流式客户端拿到一个连响应头都没有的断连，`finish` 已按 `committed` 分流）。槽位闸门在 codec 里，`decodeToolCalls` 因此改签名为 `([]ir.Event, error)`，撞线**报错而不是静默跳过**（跳过会让客户端收到少了若干工具调用的 HTTP 200，而那些调用上游确实决定要发）。**变异验证暴露一个测试缺口**：把「置位即停」的早返回删掉时测试全绿，因为既有断言撞的都是字节闸门、那时累计值本身已越线；补一格「撞块数闸门时累计字节还很小」才守住。**明确不做**：不改单帧上限（与累计闸门是两个维度）；三道阈值不做成可配（能配就能配坏，且都有三个数量级余量）；不按块分别限字节（块划分是上游的自由，限总量才是本服务关心的资源）；不做 usage 驱动的上限（`usage` 是上游自报的，等于让被限的一方自己决定限不限）；**不照搬参考仓库**（三仓正文都用 Builder/`push_str`，但两家 Go 仓的工具入参仍是 `+=`，而整条流的累计字节/帧数/块数闸门三仓全无）；调度层与配置中心零改动 |
| 1.0 | 2026-09-17 | 初稿 |
