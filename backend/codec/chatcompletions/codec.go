...
func (outboundCodec) Caps() codec.Capabilities {
	return codec.Capabilities{
		...
		// （include / truncation / max_tool_calls）外全部承载。
		Penalties:  true,
		Seed:       true,
		Candidates: true,
		LogProbs:   true,
		// verbosity 是顶层字段（responses 挪进了 text 下）。
		Verbosity: true,
		// 2026 新增的三个请求修饰槽位都有原生落点。
		...
		Citations: true,
		// message.refusal 是本协议的拒绝正文槽位：同族往返原样回到 refusal
		// 字段，客户端能区分「模型拒绝了」与「模型这么答的」。
		Refusal: true,
