...
// 流式编码的畸形入参：arguments 增量一旦发出就不可改写，原文透传
// （不延迟流），但 Notes 必须报出不可安全执行——否则客户端把一次
// 参数损坏的调用当正常完成存进历史。块正常关闭与流被掐断两条路都要报。
...
// 上游在 name 到达之前就把 index 复用给了另一次调用：前一个槽位永远
// 成不了块（半截身份直接放弃），后一次调用正常宣告，两次调用的入参
// 不许串进同一个块。
func TestReusedIndexBeforeAnnounceStartsFreshSlot(t *testing.T) {
	resp, _ := decodeStream(t,
		`{"id":"i","model":"m","choices":[{"index":0,"delta":{"tool_calls":[`+
			`{"index":0,"id":"a","type":"function","function":{"arguments":"{\"a\":"}}]}}]}`,
		`{"id":"i","model":"m","choices":[{"index":0,"delta":{"tool_calls":[`+
			`{"index":0,"id":"b","type":"function","function":{"name":"g","arguments":"{\"b\":2}"}}]}}]}`,
		`{"id":"i","model":"m","choices":[{"index":0,"delta":{},"finish_reason":"tool_calls"}]}`,
		doneSentinel,
	)
	tools := toolBlocks(resp)
	if len(tools) != 1 {
		t.Fatalf("tool blocks = %d, want 1（半截身份的前调用不成块）: %+v", len(tools), tools)
	}
	if tools[0].ToolUse.ID != "b" || tools[0].ToolUse.Name != "g" {
		t.Errorf("块的身份 = %q/%q, want b/g", tools[0].ToolUse.ID, tools[0].ToolUse.Name)
	}
	if got := tools[0].ToolUse.Input; got != `{"b":2}` {
		t.Errorf("入参 = %q，前一次调用的分片串进来了", got)
	}
}
